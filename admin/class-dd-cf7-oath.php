<?php
/**
 * OATH 2.0 for CC Plugin
 *
 * @since    1.0.0
 */

class dd_constant_contact_connector {

	const service_name = 'dd_cf7_constant_contact';
	...
	private function __construct() {
		...
		$option = get_option( 'cf7_ctct_settings' );

		if ( isset( $option['api_key'] ) ) {
			$this->api_key = $option['api_key'];
		}

		if ( isset( $option['api_secret'] ) ) {
	...
	public function get_title() {
		return __( 'Constant Contact', 'dd-cf7-plugin' );
	}
		...
		switch ( $message ) {
			case 'success':
				echo sprintf(
					...
					esc_html( __( "Connection established.", 'dd-cf7-plugin' ) )
				);
				break;
			case 'failed':
				echo sprintf(
					'<div class="error notice notice-error is-dismissible"><p><strong>%1$s</strong>: %2$s</p></div>',
					esc_html( __( "ERROR", 'dd-cf7-plugin' ) ),
					esc_html( __( "Failed to establish connection. Please double-check your configuration.", 'dd-cf7-plugin' ) )
				);
				break;
			case 'updated':
				echo sprintf(
					'<div class="updated notice notice-success is-dismissible"><p>%s</p></div>',
					esc_html( __( "Configuration updated.", 'dd-cf7-plugin' ) )
				);
				break;
		}
	}
	...
	public function display( $action = '' ) {
		echo '<p>' . sprintf(
			esc_html( __( 'The Constant Contact integration module allows you to send contact data collected through your contact forms to the Constant Contact API. You can create reliable email subscription services in a few easy steps. For details, see %s.', 'dd-cf7-plugin' ) ),
			wpcf7_link(
				__(
					'https://contactform7.com/constant-contact-integration/',
					'dd-cf7-plugin'
				),
				__( 'Constant Contact Integration', 'dd-cf7-plugin' )
			)
		) . '</p>';
		...
		if ( $this->is_active() ) {
			echo sprintf(
				...
				esc_html( __( "This site is connected to the Constant Contact API.", 'dd-cf7-plugin' ) )
			);
		}
		...
		if ( 'setup' == $action ) {
			...
			echo sprintf(
				...
				esc_html( __( 'Setup Integration', 'dd-cf7-plugin' ) )
			);
		}
	}
